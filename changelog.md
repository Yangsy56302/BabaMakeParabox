# Baba Make Parabox の 更新日志

## 1.0
**`BABA MAKE PARABOX`** 更新于 2024.07.05。
这也是**第一轮开发周期**，或者说，**早期连续开发阶段**的第一个版本。

+ 添加。
    + ~~其实上文中的“添加”指的是**Baba Make Parabox**游戏自身，怎么样，感觉很META吧？:)~~
...
+ **规则**
    + **属性**
        + **`MOVE`**
            + 添加。
            + ~~万恶之源，这之后的许多漏洞都是由于多个物体同时移动并引发冲突所引发的。~~
+ **控制**
    + 现在可以撤回操作或重新开始了。
...
+ **漏洞**
    + 修复了有关中缀修饰词的漏洞。
+ **技术性**
    + 移除了在[`v3.611`](#3611)版本用于调试的`print`函数。
...
+ **漏洞**
    + 修复了在[`v3.923`](#3923)中赋予属性会导致崩溃的漏洞。
...
##### 4.021
该版本更新于 2025.01.22，优化了游戏的控制操作和视觉提示，并修复了一些漏洞。
由于疏忽，此版本发布时并未包含可执行程序。
...
### 4.1
**`FORMAT IS UPDATE`** 更新于 2025.01.31，更改了关卡包的存储格式与运作方式。
这也是**第九轮开发周期**的第一个版本。
...
##### 4.103
该版本更新于 2025.02.02，更改了规则的存储格式以便后续版本使用，并修复了一些漏洞。

+ **规则**
    + 之前只能由`ALL IS DONE`触发的结局，从该版本开始使用部分其他方式也能触发。
+ **外观**
    + 窗口标题现在使用翻译文本。
+ **技术性**
    + **`rule`**：将游戏过程中规则列表内文本的存储方式从类改为实例。
    + **`obj.Object`**
        + `TextOn`，`TextNear`和`TextNextto`现在继承自新的`RangeInfix`类，该类拥有一个用于存储检测范围的属性变量，可用于简化部分代码。
        + 添加了`transform`方法，但并未实际代替`levelpack.Levelpack`中相似名称方法内的冗余代码。
    + **`lang`**：其他模块中所调用的`print`和`input`内置函数现在全部使用该模块内的同名变量引用。
    + **`base.versions`**：重命名为`base.version`。
+ **漏洞**
    + 修复了`FLOAT`的效果与预期不符的漏洞。
    + 修复了更改设置时，调试模式的默认值是`True`的漏洞。
    + 移除了在之前版本中用于测试的**终端**输出遗留。

##### 4.104
该版本更新于 2025.02.03，回退了[`v3.7`](#37)对否定词`NOT`所做的调整，更改了属性在游戏内的存储方式，并修复了一些漏洞。

+ **游戏**
    + 关卡转换时不再会被复位了。
+ **规则**
    + **否定词`NOT`**
        + 移除了之前版本中有意为之与**Baba Is You**不一致的设定。
+ **技术性**
    + **`obj.Properties`**
        + 部分重写。
    + `levelpack.Levelpack.transform`方法内用于转换物体的的逻辑被[`v4.103`](#4103)中加入的`obj.Object.transform`方法替代。
    + **`obj.Object.__init__`**
        + `pos`参数现在是可选项。
    + `rule.get_info_from_rule`的返回值类型标注不再为`Optional`。
+ **漏洞**
    + 修复了使用`AND`并列名词时游戏会因为调用错误的函数而导致崩溃的严重漏洞。
    + 修复了函数`bmp.lang.str_to_bool`使用了错误的方法判断`None`而导致崩溃的严重漏洞。
    + 修复了在**编辑器**内：
        + 新建关卡时，选择“是”和“否”的结果与预期相反的漏洞。
        + 新建空间时并没有使用[`v4.101`](#4101)所添加的新函数的漏洞。

#### 4.11
该版本更新于 2025.02.05，为没有用于组成规则的文字添加了透明度，添加了一些控制键位，并修复了一些漏洞。

+ **规则**
    + **属性`WIN`，`END`，和`DONE`**
        + 触发`LEVEL IS <property>`不再需要含有`YOU`性质的物体。
+ **外观**
    + 未能用于组成规则的文字现在是半透明的了。
        + 调试模式除外。
+ **控制**
    + 现在在**游戏**过程中按下鼠标中键可以在终端显示光标所在物体的信息了。
    + 现在按下`F12`可以快速切换调试模式了。
+ **技术性**
    + **`obj`**
        + **`SpriteCategory`**
            + 类型从`Literal`改为`StrEnum`。
        + **`Properties`**
            + 重命名为`PropertyInfo`，与属性词文本类`Property`区分。
+ **漏洞**
    + 修复了名词为`LEVEL`和`SPACE`的规则没有将属性应用在本体上的漏洞。
    + 修复了**终端**中分割线使用`lang.fprint`而不是`lang.print`进行输出的漏洞。
    + 再次修复了`ALL`的部分漏洞。

#### 4.111
该版本更新于 2025.03.06，重新加入了部分情况下的平滑动画，为游戏和编辑器添加了更多的信息显示，并修复了一些漏洞。
这也是**第十轮开发周期**的第一个版本。
...
+ **技术性**
    + **`render`**
        + **`calc_smooth_coord`**：添加，根据坐标和平滑插值计算中间坐标。
        + 添加了使用字符和字符串生成文本图层的函数。
        + **`gui_scale`**：重命名为`gui_scalar`。
        + **`half_gui_scale`**：重命名为`smaller_gui_scalar`。
        + **`calc_gui_scalar`**：添加，用于自动计算并更改GUI缩放倍率。
        + **`set_gui_background`**：添加，用于给GUI图层添加半透明的纯色背景。
    + **`color`**
        + 现在支持处理带有不透明度的颜色（`ColorRGBA`，`ColorHexA`）。
    + **`obj.OldObjectState`**
        + 现在会记录渲染信息（`(old|new)_surface_(pos|size)`）。
+ **漏洞**
...
+ **技术性**
    ...
    + **`obj.PropertyStorage.update`**：重命名为`add`。

#### _4.11B_
该版本重写了物体属性实际起效时所执行的逻辑。

+ **技术性**
    + 重写了使`YOU`，`MOVE`，`SHIFT`，`SELECT`，方向属性词生效的方法。
        + **漏洞**：这可能会修复一部分漏洞 *，并带来另一部分漏洞*。
    + **`obj.PropertyInfo`**
    ...
    + **`level.Level.set_space`，`levelpack.Levelpack.set_level`**：移除了用于额外设置标识符的参数。
    + **`levelpack.ReturnInfo.select`**：由于列表可以留空，该参数不再是可选项。
+ **漏洞**
