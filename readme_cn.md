...
- **文本** *（TEXT）*：组成自定义游戏规则的物体，默认包含属性`PUSH`。
    - 根据用法的不同，可以进行分类：
        - 名词：指代一类物体。
            - 诸如`BABA`，`FLAG`，`TEXT`等都是名词。
                - 注意区分所有文本和`TEXT`：所有文本都是`TEXT`名词的指代对象。
            - 如果开启元文本选项，则可以出现元文本——指向特定文本的名词，如`TEXT_BABA`。
...
- **关卡** *（LEVEL）*：可以指包含多个世界的存在，也可以指被包含在世界内的指向前者的特殊物体。约定称前者为**关卡**，称后者为**关卡物体**。
    - 一个关卡内只能有一个指向相同世界的世界物体。
        - 如果游戏过程中出现其他指向相同世界的世界物体，则随机选择一个世界物体不变，其他世界物体变为指向相同世界的克隆物体。
...
- `TEXT IS WORD`使所有文本都可以同时作为`TEXT`使用。
