...
class SpaceJson(TypedDict):
    ...
    size: bmp.loc.CoordTuple
    color: bmp.color.ColorHex
    object_list: list[bmp.obj.ObjectJson]

class Space(object):
    def __init__(self, space_id: bmp.ref.SpaceID, size: bmp.loc.Coordinate, color: Optional[bmp.color.ColorHex] = None) -> None:
        self.space_id: bmp.ref.SpaceID = space_id
        self.size: bmp.loc.Coordinate = size
    ...
    def width(self, value: int) -> None:
        self.size = bmp.loc.Coordinate(value, self.size.y)
    @property
    def height(self) -> int:
        return self.size.y
    @height.setter
    def height(self, value: int) -> None:
        self.size = bmp.loc.Coordinate(self.size.x, value)
    def out_of_range(self, coord: bmp.loc.Coordinate) -> bool:
    ...
    def new_obj(self, obj: bmp.obj.Object) -> None:
        ...
        if not self.out_of_range(obj.pos):
            self.pos_to_objs(obj.pos).append(obj)
    @auto_refresh
    def get_objs_from_pos(self, pos: bmp.loc.Coordinate) -> list[bmp.obj.Object]:
        if self.out_of_range(pos):
            return []
        return self.pos_to_objs(pos)
    @auto_refresh
    def get_objs_from_pos_and_type[T: bmp.obj.Object](self, pos: bmp.loc.Coordinate, object_type: type[T]) -> list[T]:
        if self.out_of_range(pos):
            return []
        return [o for o in self.pos_to_objs(pos) if isinstance(o, object_type)]
    @auto_refresh
    def get_objs_from_pos_and_special_noun(self, pos: bmp.loc.Coordinate, noun_type: type[bmp.obj.SupportsIsReferenceOf]) -> list[bmp.obj.Object]:
        if self.out_of_range(pos):
            return []
    ...
    @auto_refresh
    def del_objs_from_pos(self, pos: bmp.loc.Coordinate) -> bool:
        if self.out_of_range(pos):
            return False
        ...
        return deleted
    @auto_refresh
    def del_objs_from_pos_and_type(self, pos: bmp.loc.Coordinate, object_type: type) -> bool:
        ...
        return deleted
    @auto_refresh
    def del_objs_from_pos_and_special_noun(self, pos: bmp.loc.Coordinate, noun_type: type[bmp.obj.SupportsIsReferenceOf]) -> bool:
    ...
    @auto_refresh
    def get_spaces_from_pos(self, pos: bmp.loc.Coordinate) -> list[bmp.obj.SpaceObject]:
        if self.out_of_range(pos):
            return []
    ...
    @auto_refresh
    def get_levels_from_pos(self, pos: bmp.loc.Coordinate) -> list[bmp.obj.LevelObject]:
        if self.out_of_range(pos):
            return []
        return [o for o in self.pos_to_objs(pos) if isinstance(o, bmp.obj.LevelObject)]
    def get_rule_from_pos_and_direct(self, pos: bmp.loc.Coordinate, direct: bmp.loc.Direction, stage: str = "before prefix") -> tuple[list[bmp.rule.Rule], list[bmp.rule.RuleInfo]]:
    ...
    def set_rule(self) -> None:
        ...
        for x in range(self.width):
            for y in range(self.height):
                ...
                new_rule_list, new_rule_info = self.get_rule_from_pos_and_direct(bmp.loc.Coordinate(x, y), bmp.loc.Direction.D)
                ...
                new_rule_list, new_rule_info = self.get_rule_from_pos_and_direct(bmp.loc.Coordinate(x, y), bmp.loc.Direction.S)
                for rule_index in range(len(new_rule_list)):
                    part_of_old_rule = False
    ...
    def set_sprite_states(self, round_num: int = 0) -> None:
        for obj in self.object_list:
            if isinstance(obj, bmp.obj.Tiled):
                w_pos = bmp.loc.front_position(obj.pos, bmp.loc.Direction.W)
                w = len(self.get_objs_from_pos_and_type(w_pos, type(obj))) != 0
                s_pos = bmp.loc.front_position(obj.pos, bmp.loc.Direction.S)
                s = len(self.get_objs_from_pos_and_type(s_pos, type(obj))) != 0
                a_pos = bmp.loc.front_position(obj.pos, bmp.loc.Direction.A)
                a = len(self.get_objs_from_pos_and_type(a_pos, type(obj))) != 0
                d_pos = bmp.loc.front_position(obj.pos, bmp.loc.Direction.D)
                d = len(self.get_objs_from_pos_and_type(d_pos, type(obj))) != 0
                connected = {bmp.loc.Direction.W: w, bmp.loc.Direction.S: s, bmp.loc.Direction.A: a, bmp.loc.Direction.D: d}
            else:
                connected = {bmp.loc.Direction.W: False, bmp.loc.Direction.S: False, bmp.loc.Direction.A: False, bmp.loc.Direction.D: False}
    ...
    def get_stacked_transform(self, static: bmp.loc.SpaceTransform, dynamic: bmp.loc.SpaceTransform) -> bmp.loc.SpaceTransform:
        ...
        return transform
    def calc_enter_transnum(self, transnum: float, pos: bmp.loc.Coordinate, side: bmp.loc.Direction, transform: bmp.loc.SpaceTransform) -> float:
        new_side = bmp.loc.turn(side, bmp.loc.str_to_direct(transform["direct"]))
        if new_side in (bmp.loc.Direction.W, bmp.loc.Direction.S):
        ...
        return new_transnum
    def calc_leave_transnum(self, transnum: float, pos: bmp.loc.Coordinate, side: bmp.loc.Direction, transform: bmp.loc.SpaceTransform) -> float:
        new_side = bmp.loc.turn(side, bmp.loc.str_to_direct(transform["direct"]))
        if new_side in (bmp.loc.Direction.W, bmp.loc.Direction.S):
        ...
        return new_transnum
    def get_leave_transnum_from_pos(self, pos: bmp.loc.Coordinate, side: bmp.loc.Direction, transform: bmp.loc.SpaceTransform) -> float:
        new_side = bmp.loc.turn(side, bmp.loc.str_to_direct(transform["direct"]))
        if new_side in (bmp.loc.Direction.W, bmp.loc.Direction.S):
        ...
        if transform["flip"]:
            new_transnum = 1.0 - new_transnum
        return new_transnum
    def get_enter_pos_by_default(self, side: bmp.loc.Direction, transform: bmp.loc.SpaceTransform) -> bmp.loc.Coordinate:
        new_side = bmp.loc.swap_direction(side) if transform["flip"] and side in (bmp.loc.Direction.A, bmp.loc.Direction.D) else side
        new_side = bmp.loc.turn(new_side, bmp.loc.str_to_direct(transform["direct"]))
        match new_side:
            case bmp.loc.Direction.W:
                return bmp.loc.Coordinate(self.width // 2, -1)
            case bmp.loc.Direction.A:
                return bmp.loc.Coordinate(-1, self.height // 2)
            case bmp.loc.Direction.S:
                return bmp.loc.Coordinate(self.width // 2, self.height)
            case bmp.loc.Direction.D:
                return bmp.loc.Coordinate(self.width, self.height // 2)
    def get_enter_pos(self, transnum: float, side: bmp.loc.Direction, transform: bmp.loc.SpaceTransform) -> bmp.loc.Coordinate:
        new_side = bmp.loc.swap_direction(side) if transform["flip"] and side in (bmp.loc.Direction.A, bmp.loc.Direction.D) else side
        new_side = bmp.loc.turn(new_side, bmp.loc.str_to_direct(transform["direct"]))
        new_transnum = (1.0 - transnum) if transform["flip"] and side in (bmp.loc.Direction.A, bmp.loc.Direction.D) else transnum
        match new_side:
            case bmp.loc.Direction.W:
                return bmp.loc.Coordinate(int((new_transnum * self.width)), -1)
            case bmp.loc.Direction.S:
                return bmp.loc.Coordinate(int((new_transnum * self.width)), self.height)
            case bmp.loc.Direction.A:
                return bmp.loc.Coordinate(-1, int((new_transnum * self.height)))
            case bmp.loc.Direction.D:
                return bmp.loc.Coordinate(self.width, int((new_transnum * self.height)))
...
def json_to_space(json_object: SpaceJson, ver: Optional[str] = None) -> Space:
    ...
    new_space = Space(
        space_id=space_id,
        size=bmp.loc.Coordinate(*json_object["size"]),
