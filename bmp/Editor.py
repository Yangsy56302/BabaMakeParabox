...
def levelpack_editor(levelpack: bmp.Levelpack.Levelpack) -> bmp.Levelpack.Levelpack:
    ...
    current_object_type = bmp.Object.Baba
    current_direct = bmp.Locate.Direction.S
    current_cursor_pos = bmp.Locate.Coordinate(0, 0)
    current_clipboard = []
    ...
    object_type_shortcuts: dict[int, type[bmp.Object.Object]] = {k: bmp.Object.name_to_class[v] for k, v in enumerate(bmp.Base.options["object_type_shortcuts"])}
    level_changed = True
    space_changed = True
    frame = 0
    wiggle = 1
    ...
    while editor_running:
            ...
            if not current_space.out_of_range(mouse_pos_in_space):
                if mouses[0] == 1:
                    if keys["LALT"] or keys["RALT"]: # enter space; enter level (shift)
                        if keys["LSHIFT"] or keys["RSHIFT"]:
                            sub_level_objs: list[bmp.Object.LevelObject] = current_space.get_levels_from_pos(mouse_pos_in_space)
                        ...
                        if keys["LSHIFT"] or keys["RSHIFT"] or len(current_space.get_objs_from_pos(current_cursor_pos)) == 0:
                            ...
                            if issubclass(current_object_type, bmp.Object.LevelObject):
                                ...
                                level_object_extra: bmp.Object.LevelObjectExtra = {"icon": {"name": icon_name, "color": icon_color}}
                            ...
                            elif issubclass(current_object_type, bmp.Object.SpaceObject):
                            ...
                            elif issubclass(current_object_type, bmp.Object.Path):
                                unlocked = False
                ...
                # object select from list
                    ...
                    if keys["LALT"] or keys["RALT"]:
                        ...
                        obj_to_noun = bmp.Object.get_noun_from_type(current_object_type)
                        if obj_to_noun in bmp.Object.object_used:
                            current_object_type = obj_to_noun
                    else:
                        current_object_type_list = [t for t in bmp.Object.name_to_class.values() if t in bmp.Object.object_used]
                    ...
                    if keys["LALT"] or keys["RALT"]:
                        ...
                        if issubclass(current_object_type, bmp.Object.GeneralNoun) and current_object_type.ref_type in bmp.Object.object_used:
        ...
        if keys["W"] or keys["UP"]:
            current_direct = bmp.Locate.Direction.W
        elif keys["S"] or keys["DOWN"]:
            current_direct = bmp.Locate.Direction.S
        elif keys["A"] or keys["LEFT"]:
            current_direct = bmp.Locate.Direction.A
        elif keys["D"] or keys["RIGHT"]:
            current_direct = bmp.Locate.Direction.D
                ...
                if bmp.Lang.lang_input("edit.level.new") not in bmp.Lang.no:
                    ...
                    while True:
                        ...
                        try:
                            ...
                            break
                    size = bmp.Locate.Coordinate(width, height)
                    while True:
                        infinite_tier = bmp.Lang.lang_input("edit.space.new.infinite_tier")
                        try:
                            infinite_tier = int(infinite_tier) if infinite_tier != "" else 0
        ...
        # add global rule; remove global rule (shift)
            ...
            for text in text_rule:
                object_type = bmp.Object.name_to_class.get(text)
                if object_type is not None:
                    if issubclass(object_type, bmp.Object.Text):
        ...
        # cut, copy, paste
            ...
            list(map(bmp.Object.Object.reset_uuid, current_clipboard))
            current_space.del_objs_from_pos(current_cursor_pos)
        ...
        if space_changed:
            ...
            if current_cursor_pos[0] > current_space.width:
                current_cursor_pos = bmp.Locate.Coordinate(current_space.width, current_cursor_pos[1])
            if current_cursor_pos[1] > current_space.height:
                current_cursor_pos = bmp.Locate.Coordinate(current_cursor_pos[0], current_space.height)
        ...
        if issubclass(current_object_type, bmp.Object.SpaceObject):
