...
名词实际上是一种特殊的属性词，在许多情况下也可以作为属性词使用。
...
+ 将空间物体或关卡物体转变为常规物体时，会保留原本的指代信息。
+ 将常规物体转变为空间物体或关卡物体时，新的物体不会指代任何空间或关卡。
    + 例外：如果原本的物体由于某些原因还保留有相关信息，则该指代信息会被新的物体使用。
+ 将空间物体转变为关卡物体时，会创建一个只引用原空间的新关卡，作为新的关卡物体所指代的关卡。
+ 将关卡物体转变为空间物体时，原关卡所引用的全部空间都会成为新的空间物体。
...
+ **中期**
    + 加入`LINK`，关卡物体的克隆对应物——只进不出的单向链接。
    + **将指代对象存储在名词物体的实时信息内，可用的名词物体存储在空间本体的实时信息内。**
        + [`v4.11A`](changelog.md#41)部分实现。
...
+ **遥遥无期**
    + _官方关卡包……？说实话我真的不擅长设计解谜，能把这俩机制缝好就不错了……_
